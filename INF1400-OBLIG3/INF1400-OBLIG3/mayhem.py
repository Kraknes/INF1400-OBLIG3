...
class Player_Object(Sprite): # Spiller klassea
    ...
    def screenBarsUpdate(self): ## Dette kan gå inni text_file
        """
    ...
    def shoot(self):
        ...
        bullet = self.Bullet(self.angle, self.rect.centerx, self.rect.centery, 
                             self.number, self.speed_x, self.speed_y)
    ...
    def acceleration(self):
        ...
        Also loses fuel when accelerating
    ...
    class Bullet(pygame.sprite.Sprite):
        ...
        """
        def __init__(self, angle, x, y, number, vel_x, vel_y):
            """
            ...
            """
            super().__init__()
            self.player_velx = vel_x
            self.player_vely = vel_y
        ...
        def gravity(self):
            """
            Gravity influence on bullet. Does not go faster than 5 pixels in y direction.

            """
            if self.speed_y < 5:
                self.speed_y += config.GRAVITY*0.05

        def update(self, group):
            self.outOfBounds()
            self.hitPlayer(group)
            self.gravity()
            self.rect.move_ip(round(self.player_velx + self.speed_x * 10),round(self.player_vely + self.speed_y * 10))
...
class Game:
    ...
    def resetGame(self):
        ...
        """
        pygame.sprite.Group.empty(self.sprite_group) # Empties object from lists
        pygame.sprite.Group.empty(self.bullet_group)
        self.sprite_group.add(Player_Object(1, config.T_IMAGE, 30)) # Player 1 object
        self.sprite_group.add(Player_Object(2, config.T2_IMAGE, 30)) # Player 2 object
        self.player_group = self.sprite_group.sprites() # For easy access to player objects
        self.sprite_group.add(Obstacle(config.O_IMAGE, 100)) # One for each obstacle
        self.sprite_group.add(Obstacle(config.O_IMAGE, 100))
        self.sprite_group.add(Fuel(config.F_IMAGE, 50)) # Fuel cannister object

    def collisionObject(self, collision_list, player):
        for object in collision_list:
            if object != player:
                ...
                if isinstance(object, Fuel):
                    player.gainFuel()
                    object.rect.centerx = float(random.uniform(400, config.SCREEN_X-400))
                    object.rect.centery = float(random.uniform(400, config.SCREEN_Y-400))
    ...
    def playGame(self):
        ...
        while not config.DONE:
            ...
            pygame.draw.rect(pg_init.screen, config.WHITE, pygame.Rect(0, 140, config.SCREEN_X, 5)) # Upper boundary/wall
            self.sprite_group.update()
            self.bullet_group.update(self.sprite_group) 
            for player in self.player_group:
                collision_list = player.hitObject(self.sprite_group)
                self.collisionObject(collision_list, player)
